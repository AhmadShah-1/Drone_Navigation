...
class AirSimDroneEnv(gym.Env):
    def __init__(self, ip_address="", target_position=None):
        ...
        self.target_position = target_position if target_position is not None else np.array([30, 3, -10])
    ...
    def step(self, action):
        ...
        velocity = 10  # Increase the velocity
        quad_offset = {
            0: (velocity, 0, 0),  # forward
            1: (-velocity, 0, 0),  # backward
            2: (0, velocity, 0),  # right
            3: (0, -velocity, 0),  # left
            4: (0, 0, -velocity),  # up
            5: (0, 0, velocity),  # down
            6: (0, 0, 0),  # hover
        ...
        if action in [7, 8]:  # Yaw actions
            if action == 7:
                self.client.rotateByYawRateAsync(-30, 1).join()
            else:
                self.client.rotateByYawRateAsync(30, 1).join()
        else:
            self.client.moveByVelocityAsync(quad_offset[0], quad_offset[1], quad_offset[2], 1).join()
    ...
    def _compute_reward(self, velocity):
        ...
        reward += velocity  # Add velocity to reward to encourage faster movement
