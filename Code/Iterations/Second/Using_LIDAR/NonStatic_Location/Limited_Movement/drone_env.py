...
class AirSimDroneEnv(gym.Env):
    def __init__(self, ip_address="", target_positions=None, device='cpu'):
        ...
        self.observation_space = spaces.Box(low=0, high=255, shape=(360,), dtype=np.float32)  # LiDAR distances

        self.max_duration = 80  # Maximum duration of each episode in seconds
        self.ground_not_detected_start = None

        self.target_positions = target_positions if target_positions else [[30, 3, -10]]
    ...
    def reset(self, seed=None, options=None):
        ...
        self.target_position = self.target_positions[np.random.choice(len(self.target_positions))]
    ...
    def _get_obs(self):
        ...
        distances = np.array(lidar_data.point_cloud[::3], dtype=np.float32)  # Extract every third element corresponding to distances

        if len(distances) > 360:
            distances = distances[:360]
        else:
            distances = np.pad(distances, (0, 360 - len(distances)), 'constant', constant_values=np.inf)

        # Normalize distances to the range [0, 255]
        max_distance = 100  # Example maximum distance for normalization
        distances = np.clip(distances, 0, max_distance) / max_distance * 255
