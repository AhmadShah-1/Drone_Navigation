...
class AirSimDroneEnv(gym.Env):
    def __init__(self, ip_address="", target_positions=None, device='cpu'):
        ...
        self.action_space = spaces.Discrete(7)  # 6 directions + hover + yaw_left + yaw_right
        self.observation_space = spaces.Box(low=0, high=255, shape=(84, 84, 12),
                                            dtype=np.uint8)  # 4 cameras * 3 channels
    ...
    def _compute_reward(self):
        ...
        # Initial reward based on distance reduction
        if hasattr(self, 'previous_distance'):
            distance_delta = self.previous_distance - current_distance
            reward = distance_delta * 10  # Positive reward for moving closer
        else:
            reward = 0
        ...
        if hasattr(self, 'previous_action') and self.previous_action == 6:
            self.stay_still_counter += 1
            if self.stay_still_counter > 3:  # Discourage staying still for more than 3 steps
                reward -= 10
        else:
            self.stay_still_counter = 0

        self.previous_action = self.current_action

        # Small negative reward to encourage movement
        reward -= 1
